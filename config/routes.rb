...
Valkyrie::Application.routes.draw do
    ...
    api vendor_string: "mobileforce", default_version: 3 do
      ...
      version 9 do
        cache as: 'v9' do
          ...
          post '/oauth/reset_password'
          post '/oauth/accept_agreement'
          get '/oauth/suspicious_apps'

          use_doorkeeper scope: 'oauth'  do
            skip_controllers :applications, :authorized_applications, :authorizations
            controllers tokens: :oauth_tokens
            controllers token_info: :token_info if ['development', 'new_staging'].include? Rails.env
          end
        end
      end
  ...
  namespace :web_api, constraints: { format: :json }, defaults: { format: :json } do
    namespace :admin do
    ...
    resources :contracts do
      collection do
        get :check_business_contracts_ability
      end
    end
    resources :clients
    resources :partnerships, only: [:index, :new, :create]
    resources :partnership_businesses, only: :index
    ...
    match 'location_groups', to: 'location_groups#options', as: :location_groups_options, via: :options
  ...
  match 'ap_uploads', to: 'ap_uploads#options', as: :ap_uploads_options, via: :options

  resources :kpi_uploads do
    member do
      patch :cancel
      patch :submit
    end
  ...
  devise_scope :user do
    ...
    post 'users/resend_otp' => 'users/sessions#resend_otp', as: :resend_otp
  end
  ...
  resources :partnerships, only: [:new, :create, :index, :update]
  ...
  namespace :admin do
    ...
    resources :kpi_uploads, except: :new do
      member do
        post :reject
        post :start_correcting
        patch :correct
